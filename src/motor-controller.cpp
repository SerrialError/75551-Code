...
wheel_vel_bounds MotorController::get_motor_vel_bounds(const double& dt) {
    ...
    result.min = (motor.get().get_actual_velocity() * 2.0 * M_PI / 60.0 - get_motor_max_accel(-1) * dt);
    result.max = (motor.get().get_actual_velocity() * 2.0 * M_PI / 60.0 + get_motor_max_accel(1) * dt);
    return result;
}
