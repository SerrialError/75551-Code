...
void Localization::calculate_pose(void) {
    float delta_angle = DEG_TO_RAD_NORM(imu.get_rotation()) - prev_angle;
    float delta_linear_wheel = linear_wheel.get_angle() * 2.0 * 0.0254 - prev_linear_wheel;
    // Approximate the path as an arc: delta_linear_wheel is chord length, use
    // change in heading to recover local frame x/y deltas.
    float delta_x = ((delta_linear_wheel) / (DEG_TO_RAD_NORM(imu.get_rotation()) - prev_angle)) * (cos((delta_angle)) + 1.0); 
    float delta_y = ((delta_linear_wheel) / (DEG_TO_RAD_NORM(imu.get_rotation()) - prev_angle)) * (sin((delta_angle)));
